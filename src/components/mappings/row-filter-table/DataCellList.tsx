...
const DataCellList: React.FC<DataCellListProps> = ({ rowData, maxCells = 8 }) => {
  ...
  return (
    <div className="flex items-center gap-1 overflow-x-auto whitespace-nowrap max-w-full pb-0.5 scrollbar-thin">
      {displayCells.map((cell, cellIndex) => (
        <span 
          key={cellIndex} 
          className="text-xs px-1 py-0.5 bg-muted/20 rounded truncate max-w-[100px]"
          title={cell || "(empty)"}
        >
          {cell || "(empty)"}
        </span>
      ...
      {hiddenCount > 0 && (
        <span className="text-xs px-1 py-0.5 bg-sky-100 text-sky-700 rounded">
          +{hiddenCount} more
        </span>
      )}
    </div>
